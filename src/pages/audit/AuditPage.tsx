...
const AuditPage: React.FC = () => {
  ...
  const queryParams = useMemo(
    ...
    [pagination, moduleFilter, actionFilter, search, dateFrom, dateTo],
  );

  const { items, isLoading, pagination: meta } = useAuditLogs(queryParams);
  const { data: stats } = useAuditLogStats(dateFrom, dateTo);
  ...
  return (
    <div className={styles.page}>
      <PageHeader
        title="Audit Logs"
        subtitle="Track all actions performed by users in your organization."
      />
      ...
      {stats && (
        <div className={styles.statsGrid}>
          ...
          <div className={styles.statCard}>
            <span className={styles.statLabel}>Creates</span>
            <span className={styles.statValue}>
              {(stats.by_action['CREATE'] ?? 0).toLocaleString()}
            </span>
          </div>
          <div className={styles.statCard}>
            <span className={styles.statLabel}>Deletes</span>
            <span className={styles.statValue}>
              {(stats.by_action['DELETE'] ?? 0).toLocaleString()}
            </span>
          </div>
        </div>
      )}
