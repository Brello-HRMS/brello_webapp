  ...
  return (
    <div className={styles.shell}>
      <header className={styles.header}>
        ...
        {offer.offer_number && <span className={styles.offerNumberChip}>{offer.offer_number}</span>}
      </header>
          ...
          {view === 'done' ? (
            ...
            <>
              ...
              <div className={styles.offerCard}>
                  ...
                  <div className={styles.ctcBanner}>
                    <div>
                      ...
                      {offer.monthly_take_home && (
                        <span className={styles.ctcSub}>
                          ≈ ₹{Number(offer.monthly_take_home).toLocaleString('en-IN')} / month
                          take-home
                        </span>
                      )}
                    </div>
                    {offer.salary_components.length > 0 && (
                      ...
                      >
                        ...
                        <ChevronDown
                          size={14}
                          className={showBreakdown ? styles.chevronOpen : ''}
                        />
                      </button>
                    )}
                  </div>
                ...
                {showBreakdown && offer.salary_components.length > 0 && (
                    ...
                    <div className={styles.metaRow}>
                      {policies.length > 0 && (
                        <div className={styles.policiesList}>
                          {policies.map((policy) => (
                            <span key={policy.id} className={styles.policyChip}>
                              <ScrollText size={13} /> {policy.title}
                            </span>
                          ))}
                        </div>
                      )}
                ...
                {!isClosed && offer.expires_at && (
                  <div className={styles.expiry}>
                    ⏰ Expires on <strong>{new Date(offer.expires_at).toLocaleDateString()}</strong>
                  </div>
                )}
      ...
      {/* Sticky action bar — always visible, no scrolling needed to respond */}
      {view !== 'done' && !isClosed && (
      ...
      <footer className={styles.footer}>
        Powered by <strong>Brello HRMS</strong> · This link is unique to you — please don't forward
        it.
      </footer>
    </div>
  );
